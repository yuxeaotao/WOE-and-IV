...
class WoeSingleNumberic(object):
    def __init__(self, max_bins=10, woe_min=-20, woe_max=20):
        """
        初始化函数
        :param max_bins: 最大分箱数目，int
        :param min_sample_rate: 每箱最少比例，float
        :param woe_min: woe最小值
        :param woe_max: woe最大值
        """
        # self.__bins = bins
        self.__min_sample_rate = 1.0 / max_bins
    ...
    def __get_split_points(self, col):
        ...
        var_lst.sort()
        var_split_lst = [(var_lst[i] + var_lst[i+1]) / 2.0 for i in range(len(var_lst) - 1)]
        if len(var_split_lst) < self.__max_bins:
    ...
    def __woe(self, col, label):
        ...
        :return:
        """
        split_list_tmp = self.__split_list
...
class WoeSingleObject(object):
    ...
    def __get_split_list(self, col, label):
        ...
        for v in var_lst:
            ...
            label_tmp = [label[i] for i in range(len(col)) if col[i] == v] 
            var_detail_tmp.append(len(col_tmp))
            var_detail_tmp.append(np.mean(label_tmp))
            var_lst_detail_tmp.append(var_detail_tmp)
        var_lst_detail_tmp.sort(key=lambda x: x[2]) # sort by y_mean
        tmp = [[], 0]
        ...
        if tmp[1] > 0 and tmp[1] < self.__min_sample:
            if self.__var_lst_detail is None or len(self.__var_lst_detail) == 0:
                self.__var_lst_detail.append(tmp)
            elif tmp[1] >= self.__min_sample / 3:
                self.__var_lst_detail.append(tmp)
            else:
                self.__var_lst_detail[-1][0].extend(tmp[0])
                self.__var_lst_detail[-1][1] += tmp[1]
    ...
    def fit(self, df, label, **split_dict):
        ...
        for col in df.columns.tolist():
            ...
            if is_numeric_dtype(df[col]):
                ...
                woe = WoeSingleNumberic(self.__max_bins, self.__woe_min, self.__woe_max)
            else:
                self.__column_type[col] = 'object'
                woe = WoeSingleObject(self.__max_bins, self.__woe_min, self.__woe_max)
...
if __name__ == "__main__":
    train_data = pd.read_csv('/data/home/work/liupeigen/cbc/2018/q3/bcm/hand3/train_data.csv', sep='\t')
    train_x = train_data.drop(['y', 'y2', 'OrderId', 'RepayAmount', 'RepayRt', 'dt', 'RepayLastAmount'], axis=1)
    train_idx = train_data[['y', 'y2', 'OrderId', 'RepayAmount', 'RepayRt', 'dt', 'ConvertedRmb']].copy()
    train_idx.rename(columns={'ConvertedRmb': 'ori_rmb'}, inplace=True)
    train_y = train_data.y
    splits = {
    'HistoryReayAmount': [100, 250, 500, 1000, 5000],
    'HistoryRepayRt': [0.005, 0.01, 0.05, 0.1, 0.2],
    'HistoryRepayAc': [0.5,1.5,2.5],
    'benren_phone_cnt':[0.5, 1.5, 2.5, 4.5],
    'no_benren_phone_cnt': [0.5, 1.5, 2.5, 3.5, 5.5, 9.5],
    'phone_cnt': [0.5, 1.5, 2.5, 3.5, 4.5, 6.5, 9.5],
    'IdRegion': [
        ['江苏省', '辽宁省', '浙江省', '新疆维吾尔自治区'],
        ['广东省', '黑龙江省', '湖北省'],
        ['云南省', '上海市', '吉林省'],
        ['广西壮族自治区', '安徽省','陕西省', '河北省', '北京市', '福建省'],
        ['山东省', '天津市', '河南省', '四川省', '湖南省'],
        ['重庆市', '贵州省', '山西省', '甘肃省'],
        ['内蒙古自治区', '江西省', '西藏自治区', '海南省', '宁夏回族自治区', '青海省']
    ]
    }
    #测试Woe
    woe = Woe(max_bins=10)
